...
if args.oversampled:
    target_csv = "/yopo-artifact/data/dataset/from_pagegraph/target_features_rf_oversampled.csv"
else:
    target_csv = "/yopo-artifact/data/dataset/from_pagegraph/target_features_rf_full.csv"
print(target_csv)

BASE_CRAWLED_DIR = "/yopo-artifact/data/rendering_stream/html"
MODIFIED_CRAWLED_DIR = "/yopo-artifact/data/rendering_stream/modified_html_pagegraph"
...
def compute_diff():
    diff = dict()
    diff = {"FEATURE_SEMICOLON_IN_QUERY": 1, "FEATURE_URL_LENGTH": 5}
    ...
    # ordering diff
    specific_order = ["FEATURE_URL_LENGTH", "FEATURE_FROM_SUBDOMAIN", "FEATURE_FROM_THIRD_PARTY", "FEATURE_SEMICOLON_IN_QUERY", "FEATURE_RESOURCE_TYPE", "FEATURE_LOAD_TIME", "FEATURE_IN_DEGREE", "FEATURE_OUT_DEGREE", "FEATURE_IN_OUT_DEGREE", "FEATURE_MODIFIED_BY_SCRIPT", "FEATURE_PARENT_IN_DEGREE", "FEATURE_PARENT_OUT_DEGREE", "FEATURE_PARENT_IN_OUT_DEGREE", "FEATURE_PARENT_MODIFIED_BY_SCRIPT", "FEATURE_AVERAGE_DEGREE_CONNECTIVITY", "NETWORK_REQUEST_URL", "FINAL_URL"]
...
domain = ""
url = ""
perturbed_url_file = open('/yopo-artifact/scripts/crawler/pagegraph/perturbed_url_for_target_pagegraph.csv', 'w', newline='')
...
with open(target_csv, "r") as f_target:
    ...
    for line in f_target:
        ...
        # webgraph
        final_domain_with_http= line.strip().split(",")[-2]
        final_domain = final_domain_with_http.strip().split("://")[-1]
        url = line.split(",")[-3]
...
if args.oversampled:
    ...
    df_oversampled = pd.read_csv('/yopo-artifact/data/dataset/from_pagegraph/target_features_rf_oversampled.csv')
    filtered_df = df_oversampled.iloc[filtered_indices]
    filtered_df.to_csv('/yopo-artifact/data/dataset/from_pagegraph/target_features_rf_full.csv', index=False)

    # Exclude unnecessary columns
    col_exclude = ["NETWORK_REQUEST_URL", "FINAL_URL"]
    filtered_df_excluded = filtered_df.drop(columns=col_exclude)

    filtered_df_excluded.to_csv('/yopo-artifact/data/dataset/from_pagegraph/target_features_rf.csv', index=False)
